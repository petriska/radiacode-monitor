...
# RadiaCode monitor
<img width="1102" height="867" alt="radiacodeGUI030" src="https://github.com/user-attachments/assets/d26edc4b-1f68-4a23-a491-a7416673766b" />
...
- **Background / Net**: **Load BG** from CSV/TKA/N42/NPES-JSON (view modes unlock after load)
- **Waterfall** under spectrum: count-rate vs time (ΔN/Δt), follows X zoom; integrate N polls per row
...
parent/
  ...
  radiacode-monitor-0.3.0/  # extracted app sources
```

```bash
cd radiacode-monitor-0.3.0   # extracted app folder
mkdir build && cd build
cmake .. -DCMAKE_PREFIX_PATH=/path/to/Qt/6.x
...
.\scripts\package-windows.ps1 `
  ...
  -Version 0.3.0   # optional; omit to use CMakeLists.txt
```
...
# ./scripts/package-deb.sh --build-dir build-deb --version 0.3.0
```
